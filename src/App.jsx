  ...
  return (
    <ReactLenis root>
      ...
      <main className="flex flex-col w-full m-0 p-0 space-y-0">
        ...
        <nav className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-6 py-8 md:py-4 transition-all duration-300 bg-white/90 backdrop-blur-md border-b border-antique-gold/20">
           <div className="flex items-center flex-shrink-0">
             <a
               href="#rsvp-section"
               className="bg-gradient-to-r from-antique-gold to-[#FCF6BA] text-white px-5 md:px-6 py-3 md:py-2 rounded-full font-bold shadow-lg marshmallow-transform font-cinzel text-xl md:text-base whitespace-nowrap"
             >
               SVAR
             </a>
           </div>

           {/* Countdown - Mye større på mobil, sentrert */}
           <div className="absolute left-1/2 transform -translate-x-1/2 flex items-center gap-2 md:gap-3">
             {[
               ...
               <React.Fragment key={item.label}>
                 ...
                 <div className="text-center bg-white/90 backdrop-blur-sm border-2 border-[#D4AF37]/40 rounded-lg px-3 md:px-4 py-2.5 md:py-3 shadow-lg hover:shadow-xl transition-all duration-300">
                   <div 
                     className="font-cinzel text-lg md:text-3xl font-black tracking-tight"
                     style={{
                   ...
                   <div className="text-[9px] md:text-[10px] font-montserrat text-[#D4AF37] font-semibold tracking-wider uppercase mt-1">{item.label}</div>
                 </div>
               </React.Fragment>
             ))}
           </div>
           ...
           <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
             <button
               ...
               className="p-2.5 md:p-2 text-deep-charcoal hover:text-antique-gold marshmallow-transform"
               aria-label="Toggle menu"
             >
               {menuOpen ? <X size={28} className="md:w-7 md:h-7" /> : <Menu size={28} className="md:w-7 md:h-7" />}
             </button>
           </div>
        </nav>
